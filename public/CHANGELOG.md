...
- Refined the visual theme with eased 0.5s transitions for track and pattern elements, better font color styling for instrument-colored steps in rainbow mode, and a subtle fix to the instrument name input background so focused and hovered states feel more consistent.
...
- Upgraded the **Electron** runtime from 28.x to 39.x, including updated Node typings and supporting docs describing the upgrade and compatibility checks for `main.cjs` and `preload.cjs`, while keeping the existing audio and UI behavior unchanged.
...
- Introduced a track background color toggle with rainbow icons and simplified gradient styling so active tracks and background modes are easier to distinguish at a glance, including padding and alignment tweaks and a single active-line border to reduce visual noise.
...
- Refined the command panel and button styling by adding themed gradients to NEW/LOAD/SAVE and other controls, softening hover borders, tweaking MIDI and DEBUG and RESET colors, adjusting logo and header positioning and swapping CLONE and DUPLICATE line buttons to better match shallow vs deep copy semantics.
- Improved playback and export behavior by fixing a song loop timing bug in the debug and loop logic so looped playback matches the tracker grid, and reordering DATA and DUMP buttons in the Export modal to match the documented specification.
- Continued internal refactoring by extracting MIDI and instrument action handlers into dedicated hooks, simplifying `useDataManagement` dependencies, adding mobile-focused piano and command layout tweaks and introducing long-pattern song export fixtures and related tests.
...
- Reorganized the command panel into three clearer rows that match the tracker workflow, renamed several buttons (for example ADD TRACK and ADD INST and BUG), and introduced a **DELETE TRACK** action that clears the pattern assignment for the selected track on the current playlist line without deleting the underlying pattern.
- Improved export feedback by showing information dialogs after DATA/BIN/VGM/WAV exports (matching existing DUMP/WAV behavior) with details such as filename, scope and byte and sample counts, and fixed dialog z-ordering so these messages now appear above the Export window.
- Polished UI styling and internals by adjusting export modal layout and radio-button alignment, refining MIDI and debug button colors, tightening debug logging and React effect dependencies, and replacing remaining `any` types and console ESLint overrides with stronger TypeScript definitions.
...
- Relaxed unsaved-changes protection by removing quit and reload confirmation dialogs in Electron and browser builds, so closing the app or refreshing no longer shows a warning when edits are pending.
...
- Added **per‑instrument MIDI output** with configurable channel and program settings, keyboard shortcuts, and explicit port open and close lifecycle management so instruments can route to multiple external devices without locking MIDI ports.
- Added track‑focused live MIDI recording that writes note‑on and note‑off events directly into the current pattern, keeping playback and recording in sync with the tracker grid.
...
- Introduced **YAML‑based MIDI configuration import and export** with error‑handling dialogs, plus a dedicated MIDI System Reset button and automatic System Reset on output activation to keep external devices in a known state.
- Standardized YAML song and clipboard formats by renaming `steps` to `step`, replacing legacy `off: true` flags with explicit `note: OFF` entries, omitting empty pattern step arrays, and updating example songs to follow the canonical schema (note: new track clipboard exports are not backward‑compatible with older `off` and `steps` formats).
- Enhanced exports by fixing sustain envelope handling and sequencer worker tick ordering for VGM and WAV output, and adding GD3 tags (including game name and song metadata encoded in UTF-16LE) to VGM files.
- Refactored the application layout, modals and core hooks (state, keyboard shortcuts, scroll sync, audio setup, playlist and track and file operations, playback controls and MIDI handling) into dedicated modules and tightened TypeScript types to reduce App.tsx complexity and improve long‑term maintainability without changing workflows.
...
- Made MIDI configuration changes (input and output device selection and ignore-volume toggles) apply instantly when changed in the MIDI modal, instead of only on save, so routing tweaks take effect immediately.
...
- Introduced experimental **MIDI support** with configurable input and output devices, device selection and real-time monitoring so external keyboards and synths can drive DOSOUND Tracker.
- Added live MIDI playback features including enable and disable toggle, debug console logging of MIDI input and output events, note transposition based on instrument base pitch, and improved disabled‑state styling on MIDI controls.
...
- Enhanced Windows packaging and icons with a dedicated Electron and NSIS configuration, multi‑resolution icon support, an updated 256×256 PNG icon, and documentation covering ICO creation requirements and tool limitations.
...
- Simplified instrument envelope property names from `volumeEnvelope` and `arpeggioEnvelope` and `pitchEnvelope` and `modeEnvelope` to shorter `volume` and `arpeggio` and `pitch` and `mode`, and introduced a one‑time localStorage migration to clear legacy keys so saved instruments remain compatible.
- Extended the **tone/noise mode** selector to support three states (tone, noise, both) with a purple highlight for combined mode, added `T` and `N` and `B` keyboard shortcuts for quick mode selection, and refined envelope panel padding for a more compact layout.
...
- Made editor interactions more robust by forcing scrollbar visibility in the instrument list to avoid layout shifts, enabling vertical scrolling in the playlist content area, adding padding around song info, and stopping event propagation from INSERT STEP and DELETE STEP buttons to prevent unintended parent interactions.
- Refreshed overall **UI and documentation** with updated logo and icon styling and interaction polish, a dedicated debug mode information modal with clearer dump panel copy, a comprehensive data format reference for all save and export formats, and a full operational manual aimed at composers and assembly coders.
...
- Cleaned up song YAML export by omitting empty title and author fields and invalid year values, removing redundant pattern name fields, and keeping the saved format focused on data that actually matters.
- Enhanced the **instrument list workflow** with dynamic slot display, instrument reordering via up and down buttons and Ctrl and Arrow shortcuts that automatically remap instrument IDs in all patterns, and a deletion confirmation dialog that shows where an instrument is used before you remove it.
...
- Refined the transport and command panel UI with accent-colored playback buttons, themed gradients and glows, idle-state styling, reordered dump and export buttons, and clearer labels (including renaming **EXPORT SOUND** to **EXPORT WAV** and simplifying instrument and playlist header text).
...
- Expanded test coverage and documentation with new unit tests for instrument and register-dump exports (including noise envelope validation) and several refactoring and code-review documents that outline future cleanup work.
...
- Enhanced the volume envelope editor and piano preview: sustain points are shown as black dot markers with right‑click and `S` shortcuts to toggle sustain, piano keyboard preview is now strictly monophonic with robust key‑up handling, and preview automatically stops when a released note reaches zero volume.
...
- Streamlined playlist editing with move up and down buttons and Ctrl and Arrow keyboard shortcuts to reorder playlist lines, removing legacy **GOTO** commands from the playlist, and letting track header clicks jump the playlist cursor to the selected track.
...
- Introduced a **Notes** panel with rotating demoscene and tracker philosophy messages, click‑to‑advance behavior and many new messages, and adjusted the window layout and minimum size so Notes and editor panels fit cleanly alongside the changelog.
- Reworked information and confirmation and export modals to use reusable dialog components, normalized escaped `\n` sequences to real newlines so multi‑line messages render correctly, and overhauled the README and internal docs with expanded usage and refactoring guidance.
...
- Improved keyboard and mouse navigation in the pattern editor so left and right arrows move between note and volume columns across tracks and clicking a cell selects its column with clear visual highlighting.
...
- Improved the **CHANGES** modal with better spacing and added global keyboard shortcuts (ESC and ENTER) for closing or confirming dialogs.
...
- Reworked **EXPORT SOUND** so WAV export uses the same YM2149-based synthesis path as PLAY SONG and PLAY PATTERN (tone, noise, envelopes and timing now match runtime playback).
...
- Replaced the sequencer `setInterval` loop with a high‑precision `setTimeout` loop that tracks `lastTickTime` and `nextTickTime` to reduce timer drift.
...
- Removed playlist line move up and down buttons and footer controls from `PlaylistPanel` to simplify the UI.
- Reorganized `SongInfoPanel` into a two‑column grid: Year and Speed on the left, Length and Loop on the right.
- Adjusted the main layout so the right column uses a slightly smaller flex ratio for better overall balance.
- Added null and empty handling for the loop field so cleared values are treated safely throughout the app.

## Version 1.0.16

- Added a **description** field so songs can carry a short textual description alongside title and author.
